...
function likePost() {
  ...
  for (let i = 0; i < likebttn.length; i++) {
    likebttn[i].addEventListener('click', (e) => {
      let likeid = likebttn[i].id;
      let countLike = parseInt(e.target.dataset.like);
      countLike++;
      console.log(likeid, countLike)
    })
  }
}
...
var counter = 0;
var button = newFunction();
button.addEventListener('click', () => {
  counter++;
  if (counter === 1) {
    button.innerHTML = counter + ' like';
  } else {
    button.innerHTML = counter + ' likes'
  }
})
...
// })
//        <button id='${doc.id}' class="like"  onClick="likePost()" data-like=${doc.data().like}'>like</button>
...
function newFunction() {
  return document.getElementsByClassName("likebttn");
}
